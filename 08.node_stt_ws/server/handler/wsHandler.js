...
function webSocketServer(_server) {
    ...
    wss.on('connection', async (ws, req) => {
        ...
        ws.on('message', (msg) => {
            ...
            check_tid(data.tid).then( async (check) => {
                    ...
                    if(sound_data.length < 1000){
                        try {
                            ...
                            if(fs.existsSync(`${savePath}/get_sound_from_Client.wav`)){
                                let tid_info = {tid : data.tid, create_time : check.create_time}
                                fs.writeFileSync(`${savePath}/tid_info.txt`, JSON.stringify(tid_info))
                                ...
                                if(stt_text){
                                    ...
                                    const stt_data = {stt : JSON.parse(stt_text), certified : 1, tid : data.tid, create_time : check.create_time,fileSend : data.fileSend};
